...
from unifier.blocks.losses import ConVIRTLoss, InfoNCELoss
...
class ConVIRT(nn.Module):
    def __init__(
        self, encoder, cnn, projection, loss, forward_batch_size=256, **kwargs
    ):
        super().__init__()
    ...
    def forward(self, input_ids, attention_mask, images, **kwargs):
        ...
        for i in list(chunks(range(bs), min(self.fbs, bs))):
            ...
            linguistic = self.linguistic(input_ids=inp, attention_mask=att, **kwargs)
