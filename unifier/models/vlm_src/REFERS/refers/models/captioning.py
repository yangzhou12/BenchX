...
from ..data.structures import ImageCaptionBatch
from ..data.tokenizers import SentencePieceBPETokenizer
from ..modules.textual_heads import TextualHead
from ..modules.visual_backbones import VisualBackbone
from ..modules.Fusion import fusion
from ..modules.contrastive_learning import TextEncoder
from ..utils.beam_search import AutoRegressiveBeamSearch
from ..utils.losses import LossesOfConVIRT
...
#-----------------------------------------------------------------------------------------
    ...
    def forward_embeddings(self, imgs=None, texts=None):
         # shape: (batch_size, channels, height, width)
        visual_features = self.visual(imgs)
        global_feature = visual_features[:, 0, :].unsqueeze(1)
        visual_features = visual_features[:, 1:]
        visual_features = visual_features + global_feature

        visual_features2 = self.visual(imgs)
        global_feature2 = visual_features2[:, 0, :].unsqueeze(1)
        visual_features2 = visual_features2[:, 1:]
        visual_features2 = visual_features2 + global_feature2

        visual_features3 = self.visual(imgs)
        global_feature3 = visual_features3[:, 0, :].unsqueeze(1)
        visual_features3 = visual_features3[:, 1:]
        visual_features3 = visual_features3 + global_feature3
        ...
        visual_features_all =  torch.cat((visual_features, visual_features2), 1)
        visual_features_all =  torch.cat((visual_features_all, visual_features3), 1)
        ...
        del texts["cap_lens"]
        caption_feature = self.contrastive_bert(texts)

        return {"img_emb_g": global_feature_all, "text_emb_g": caption_feature}
