...
class MedCLIPModel(nn.Module):
    ...
    def from_pretrained(self, ckpt=None):
        '''
        ...
        if not os.path.exists(ckpt): # if checkpoint does not exist
            if not os.path.exists(input_dir): # if checkpoint root dir does not exist
