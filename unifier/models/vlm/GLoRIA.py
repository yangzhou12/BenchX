...
from unifier.blocks.losses import GLoRIALoss, gloria_attention_fn, cosine_similarity
...
def evaluation(models, config, dl, from_training, **kwargs):
    ...
    return {
        "loss": np.ndarray.mean(np.array(losses)),
...
class GLoRIA(nn.Module):
    def __init__(
        self, encoder, cnn, visual_embedder, loss, dl, forward_batch_size=12, **kwargs
    ):
        super().__init__()
        ...
        self.global_embedder = nn.Linear(
            visual_embedder.feature_dim, self.linguistic.encoder.config.hidden_size
        )
        self.local_embedder = nn.Conv2d(
            visual_embedder.interm_feature_dim,
    ...
    def forward(self, input_ids, attention_mask, images, **kwargs):
        ...
        for i in list(chunks(range(bs), min(self.fbs, bs))):
            ...
            global_features.append(
                self.global_embedder(self.visual(self.up_sample(images_.cuda())))
            )
            local_features.append(
                self.local_embedder(self.activation["local_features"])
            )
            output = self.linguistic(
                input_ids_.cuda(), attention_mask_.cuda(), output_hidden_states=True
            )
        ...
        # Losses
        loss, attention_maps = self.loss_fn(
            global_features, local_features, word_embeddings, sent_embeddings, sents
        )
    ...
    def get_similarities(
        self, input_ids, attention_mask, images, similarity_type="both"
    ):
        # warnings
        ...
        img_emb_g, img_emb_l, text_emb_l, text_emb_g = (
            out["global_features"],
            out["local_features"],
            out["word_embeddings"],
            out["sent_embeddings"],
