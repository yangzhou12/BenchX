...
class LabelSmoothingCrossEntropy(nn.Module):
    ...
    def forward(self, output, target):
        B, c = output.size()
        log_preds = F.log_softmax(
            output, dim=-1
        )  # log_preds should be reduced to [32, 1] here
        if self.reduction == "sum":
            ...
            if self.reduction == "mean":
                loss = loss.mean()
        return loss * self.smoothing / c + (1 - self.smoothing) * F.nll_loss(
...
class LabelSmoothingCrossEntropyWithSuperLoss(nn.Module):
    ...
    def __repr__(self):
        s = (
            "LabelSmoothingCrossEntropyWithSuperLoss (eps="
            + str(self.eps)
            + ", reduction="
            + self.reduction
            + ", classes="
...
class MixUpLoss(nn.Module):
    ...
    def forward(self, pred, label, label_mixed, lam):
        ...
        lam = lam.cuda()
        return lam * self.criterion(pred, label) + (1 - lam) * self.criterion(
            pred, label_mixed
        )
