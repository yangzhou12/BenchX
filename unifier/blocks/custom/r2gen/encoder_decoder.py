...
class EncoderDecoder(AttModel):
    ...
    def __init__(self, transformer):
        super(EncoderDecoder, self).__init__(transformer)
        self.args = transformer
        self.num_layers = transformer.num_layers
        self.d_model = transformer.d_model
        self.d_ff = transformer.d_ff
        self.num_heads = transformer.num_heads
        self.dropout = transformer.dropout
        self.rm_num_slots = transformer.rm_num_slots
        self.rm_num_heads = transformer.rm_num_heads
        self.rm_d_model = transformer.rm_d_model

        tgt_vocab = self.vocab_size + 1

        self.model = self.make_model(tgt_vocab)
        self.logit = nn.Linear(transformer.d_model, tgt_vocab)
