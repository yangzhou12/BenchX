...
def test_custom_dataset():
    ...
    eval_results = train_dataset.evaluate(
        pseudo_results, metric=["mIoU", "mDice", "mFscore", "medDice"]
    )
    assert isinstance(eval_results, dict)
    assert "mIoU" in eval_results
    assert "mDice" in eval_results
...
def test_eval_concat_custom_dataset(separate_eval):
    ...
    eval_results2 = dataset2.evaluate(
        pseudo_results * 2, metric=["mIoU", "mDice", "mFscore", "medDice"]
    )
    ...
    eval_results1 = dataset1.evaluate(
        eval_results1, metric=["mIoU", "mDice", "mFscore", "medDice"]
    )
    ...
    eval_results2 = dataset2.evaluate(
        eval_results2, metric=["mIoU", "mDice", "mFscore", "medDice"]
    )
