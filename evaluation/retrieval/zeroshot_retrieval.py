...
def get_tokenizer(args):
    ...
    if args.model_name in ["mrm"]:
        tokenizer = PreTrainedTokenizerFast(
            tokenizer_file="/home/faith/projects/unified-framework/models/mrm/mimic_wordpiece.json",
...
def get_tokenizer(args):
    ...
    if args.model_name in ["mrm"]:
        tokenizer = PreTrainedTokenizerFast(
            tokenizer_file="/home/faith/unified-framework/models/mrm/mimic_wordpiece.json",
            add_special_tokens=True,
            pad_token="[PAD]",
        )
        return tokenizer
...
def evaluate(predictions, targets):
    ...
    for k in [1, 5, 10]:
        ...
        for i in range(num_samples):  # for each text/image query
            top_k_predictions = predictions[i][
                :k
            ]  # extract top k candidate images/reports
            hit = False
            for idx in top_k_predictions:  # for each candidate
                if torch.equal(
                    targets[idx], targets[i]
                ):  # if class of query matches class of candidate
                    num_correct += 1
    ...
    return {"Hit@K": hit_at_k, "Precision@K": precision_at_k}
...
def main(args):
    ...
    print(
        f"Hit@1: {H1}, Hit@5: {H5}, Hit@10: {H10}, \n",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--retrieve_images",
        action="store_true",
        help="switch to text-to-image retrieval instead of image-to-text",
    )
