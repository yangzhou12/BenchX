...
class SIIMDistributedSampler(_DistributedSampler):
    ...
    def __init__(
        ...
        for i in range(len(self.dataset)):
            self.filenames.append(
                self.dataset.img_infos[i]["filename"].replace(".png", "")
            )

        df = pd.read_csv(
            "/home/faith/projects/mmsegmentation/mmseg/datasets/DatasetsSplits/siim.csv"
        )
        ...
        self.pos_indices = [
            element_indices.get(index, [None])[0] for index in self.pos_filenames
        ]
        self.pos_indices = [x for x in self.pos_indices if x is not None]
        self.pos_indices = list(set(self.pos_indices))
        self.neg_indices = [
            element_indices.get(index, [None])[0] for index in self.neg_filenames
        ]
    ...
    def __iter__(self) -> Iterator:
        ...
        if self.shuffle:
            ...
            neg_select = torch.randperm(len(self.neg_indices), generator=g).tolist()[
                : round(self.nums_pos * 0.4)
            ]
