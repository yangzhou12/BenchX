...
try:  # load train set queries if possible
    ...
    for h in np.unique(mimicdescs["Variable"].values):
        mimlist.append(
            mimicdescs[mimicdescs["Variable"] == h].sample(30, random_state=0)
        )
...
class MedCLIP(nn.Module):
    def __init__(self, freeze_transformer=False, eval=True, freeze_CNN=False):
        ...
        self.cnn = (
            Vision_Model.get_biovil_resnet()
        )  # ResNet-50 as used in BioViL - pretrained (img encoder)
        ...
        self.cls_projection_head = (
            self.transformer.cls_projection_head
        )  # BertProjectionHead to be used with BERT CLS token
        ...
        for param in self.transformer.parameters():
            param.requires_grad = True
        modules = [
            self.transformer.bert.embeddings,
            *self.transformer.bert.encoder.layer[:8],
        ]
    ...
    def get_im_embeddings(self, images, only_patches=False, only_ims=False):
        ...
        for im in images:
            output = self.cnn(im)
            patch_embs = (
                output.projected_patch_embeddings
            )  # N E P1 P2  #dataclass ImageOutput
    ...
    def encode_text(
        self, input_ids, attention_mask, only_words=False, only_texts=False
    ):  # custom proxy function for zeroshot classification
        text_output = self.transformer(input_ids, attention_mask)
        text_emb = self.transformer.get_projected_text_embeddings(
            input_ids=input_ids, attention_mask=attention_mask
        ).to(device)

        # get word embeddings using transformer projection head (predict [CLS] token)
        word_embs_projected = [
            self.cls_projection_head(text_output.last_hidden_state[:, i, :])[:, None, :]
            for i in np.arange(text_output.last_hidden_state.shape[1])
        ]
    ...
    def get_text_embeddings(self, text, only_words=False, only_texts=False):
        ...
        token_output = self.tokenizer.batch_encode_plus(
        ...
        text_output = self.transformer(
            token_output.input_ids, token_output.attention_mask
        )

        # get text embeddings from transformer
        text_emb = self.transformer.get_projected_text_embeddings(
            input_ids=token_output.input_ids, attention_mask=token_output.attention_mask
        ).to(device)
    ...
    def get_cross_weights(self, all_patches, word_embs):
        ...
        word_embs = word_embs / word_embs.norm(
            dim=2, keepdim=True
        )  # normalized word embeddings
        stack_patches = torch.cat(all_patches, dim=0).to(device)  # N x E x P1 x P2
        stack_patches = stack_patches.reshape(
            stack_patches.shape[0], E, -1
        )  # N x E x P --> P = P1 x P2
        ...
        cross_weights_text = torch.bmm(
            word_embs.repeat(len(all_patches), 1, 1), stack_patches
        )
        ...
        return (
            cross_weights,
            stack_patches,
        )  # list containing 1 element - just NTP object
    ...
    def forward(self, images, text=None):
        ...
        all_patches, all_im_embs = self.get_im_embeddings(
            images
        )  # removed all_pool_weights
        ...
        return (
            ...
            stack_patches,
            word_embs,
        )  # list per im, #list per im, #list per im-im pair


class SimClassifier(nn.Module):
    def __init__(
        self, heads, clip_model=None, image_grad=False, transformer_grad=False
    ):
        super().__init__()
        ...
        self.neg_label_embeds = neg_label_embeds / label_embeds.norm(
            dim=1, keepdim=True
        )
    ...
    def forward(self, image):  # batch of images, not lists
        list_im_embeds = self.clip_model.get_im_embeddings(
            [image], only_ims=True
        )  # N E
        list_im_embeds = [
            im_embeds / im_embeds.norm(dim=1, keepdim=True)
            for im_embeds in list_im_embeds
        ]  # N E
        # N E x c E = N c
        list_preds = [im_embeds @ self.label_embeds.t() for im_embeds in list_im_embeds]
        list_neg_preds = [
            im_embeds @ self.neg_label_embeds.t() for im_embeds in list_im_embeds
        ]
        for i, preds in enumerate(list_preds):
            neg_preds = list_neg_preds[i]
            preds = torch.stack(
                [preds[:, :, None], neg_preds[:, :, None]], dim=2
            )  # N C 2
        ...
        if checkpoint:
            ...
            if num_models > 1:
                checkpoint = torch.load(
                    modelpath + "best_model_" + str(i) + ".pt", map_location=device
                )
            else:
...
def getLabelEmbeddings(
    clip_model,
    heads,
    labeldescs=labeldescs,
    avg=True,
    convirt=True,
    customdescs=[],
    getneg=False,
    ...
    for h in heads:  # for chexpert and covid19 mainly
        ...
        if getneg:
            desc = np.append(
                desc, np.array(["No " + h + "."])
            )  # add to the no finding queries
        head_embeddings = clip_model.get_text_embeddings(list(desc), only_texts=True)
        lab_embeddings[h] = (
            torch.mean(head_embeddings, dim=0) if avg else head_embeddings
        )
