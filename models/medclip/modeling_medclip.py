...
class SuperviseClassifier(nn.Module):
    ...
    def __init__(
        self, vision_model, num_class=14, input_dim=768, mode=None, **kwargs
    ) -> None:
        """args:
...
class PromptTuningClassifier(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.prev_n_tokens = (
            self.model.text_model.model.embeddings.word_embeddings.num_embeddings
        )
        ...
        if not joint_train_emb:
            self.model.text_model.model.embeddings.word_embeddings = (
                PartiallyFixedEmbedding(
                    fixed_weights=self.prev_embeddings, num_to_learn=self.n_new_tokens
                )
            )
        else:
